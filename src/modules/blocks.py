...
class DiTBlock(nn.Module):
    def __init__(
        ...
        self.self_attn = SelfAttention(
            dim, num_heads, qkv_bias, attn_drop_rate, proj_drop_rate, use_xformers
        )

        # --- Temporal attention (frames) ---
        self.norm_temp_attn = norm_layer(dim)
        self.temp_attn = TemporalAttention(
            dim, num_heads, qkv_bias, attn_drop_rate, proj_drop_rate, use_xformers, use_flow=True
        )

        # # --- Flow cross-attention (NEW) ---
        # self.norm_flow_attn = norm_layer(dim)
        # self.flow_attn = TemporalAttention(
        #     dim, num_heads, qkv_bias, attn_drop_rate, proj_drop_rate, use_xformers
        # )
    ...
    def forward(
        ...
        # print("After concatenation, K shape:", K.shape, "V shape:", V.shape)

        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = \
            modulations.chunk(6, dim=1)

        # 1. Self-attention
        query_tokens = query_tokens + gate_msa.unsqueeze(1) * self.self_attn(
            modulate(self.norm_self_attn(query_tokens), shift_msa, scale_msa)
        )

        # 2. Temporal attention (frames)
        query_tokens = query_tokens + self.temp_attn(
            self.norm_temp_attn(query_tokens),
            x0=tokens0,
            x1=tokens1,
            flow_fwd_tokens=flow_fwd_tokens,
            flow_bwd_tokens=flow_bwd_tokens,
            ph=ph,
            pw=pw
        )

        # 4. MLP
        query_tokens = query_tokens + gate_mlp.unsqueeze(1) * self.mlp(
            modulate(self.norm_mlp(query_tokens), shift_mlp, scale_mlp)
        )
        return query_tokens
